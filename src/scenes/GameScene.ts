  ...
  create(): void {
    ...
    this.input.on("pointermove", (pointer: Phaser.Input.Pointer) => {
      ...
      if (this.ghostTower && this.selectedTowerType) {
        ...
        const canPlace =
          !this.isOnPath(position) && this.gold >= (level1?.cost || 0);
  ...
  private updateUpgradeIndicators(): void {
    ...
    if (this.hoveredTower && this.hoveredTower.canUpgrade()) {
      ...
      if (container) {
        ...
        container.y =
          this.hoveredTower.sprite.y -
          (this.hoveredTower.sprite.displayHeight / 2 - 16);
        ...
        if (priceText)
          priceText.setText(
            `${this.buildLevelText(this.hoveredTower)}: ${cost}`
          );
